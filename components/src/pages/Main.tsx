...
const Main = () => {
  ...
  const listOfCards = (searchCards as Array<IDataApi>)
    .filter((item) => item.name.toLowerCase().trim().includes(`${valueSearchInput.toLowerCase()}`))
    .map((item, index) => {
  ...
  return (
    <div className="main">
      ...
      <div className="card-block" id="card-block" data-testid="card">
        ...
        {errorMessage}
        <Portal>
          <Modal activeModal={activeModal} setActive={setModalActive} selectedCard={selectedCard} />
        </Portal>
      </div>
    </div>
  );
